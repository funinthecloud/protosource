// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.10
// 	protoc        (unknown)
// source: acme/app/sample/v1/sample_v1.proto

package samplev1

import (
	_ "github.com/funinthecloud/protosource/options/v1"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// Sample is the aggregate.
//
//	It must be defined first in the proto,
//	its name must match the first part of the package.
type Sample struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Version       int64                  `protobuf:"varint,2,opt,name=version,proto3" json:"version,omitempty"`
	CreateAt      int64                  `protobuf:"varint,3,opt,name=create_at,json=createAt,proto3" json:"create_at,omitempty"`
	CreateBy      string                 `protobuf:"bytes,4,opt,name=create_by,json=createBy,proto3" json:"create_by,omitempty"`
	ModifyAt      int64                  `protobuf:"varint,5,opt,name=modify_at,json=modifyAt,proto3" json:"modify_at,omitempty"`
	ModifyBy      string                 `protobuf:"bytes,6,opt,name=modify_by,json=modifyBy,proto3" json:"modify_by,omitempty"`
	Body          string                 `protobuf:"bytes,7,opt,name=body,proto3" json:"body,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Sample) Reset() {
	*x = Sample{}
	mi := &file_acme_app_sample_v1_sample_v1_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Sample) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Sample) ProtoMessage() {}

func (x *Sample) ProtoReflect() protoreflect.Message {
	mi := &file_acme_app_sample_v1_sample_v1_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Sample.ProtoReflect.Descriptor instead.
func (*Sample) Descriptor() ([]byte, []int) {
	return file_acme_app_sample_v1_sample_v1_proto_rawDescGZIP(), []int{0}
}

func (x *Sample) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *Sample) GetVersion() int64 {
	if x != nil {
		return x.Version
	}
	return 0
}

func (x *Sample) GetCreateAt() int64 {
	if x != nil {
		return x.CreateAt
	}
	return 0
}

func (x *Sample) GetCreateBy() string {
	if x != nil {
		return x.CreateBy
	}
	return ""
}

func (x *Sample) GetModifyAt() int64 {
	if x != nil {
		return x.ModifyAt
	}
	return 0
}

func (x *Sample) GetModifyBy() string {
	if x != nil {
		return x.ModifyBy
	}
	return ""
}

func (x *Sample) GetBody() string {
	if x != nil {
		return x.Body
	}
	return ""
}

// Create is the command that is used to create a sample.
type Create struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Actor         string                 `protobuf:"bytes,2,opt,name=actor,proto3" json:"actor,omitempty"`
	Body          string                 `protobuf:"bytes,3,opt,name=body,proto3" json:"body,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Create) Reset() {
	*x = Create{}
	mi := &file_acme_app_sample_v1_sample_v1_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Create) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Create) ProtoMessage() {}

func (x *Create) ProtoReflect() protoreflect.Message {
	mi := &file_acme_app_sample_v1_sample_v1_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Create.ProtoReflect.Descriptor instead.
func (*Create) Descriptor() ([]byte, []int) {
	return file_acme_app_sample_v1_sample_v1_proto_rawDescGZIP(), []int{1}
}

func (x *Create) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *Create) GetActor() string {
	if x != nil {
		return x.Actor
	}
	return ""
}

func (x *Create) GetBody() string {
	if x != nil {
		return x.Body
	}
	return ""
}

// Update is the command that is used to update a sample.
type Update struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Actor         string                 `protobuf:"bytes,2,opt,name=actor,proto3" json:"actor,omitempty"`
	Body          string                 `protobuf:"bytes,3,opt,name=body,proto3" json:"body,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Update) Reset() {
	*x = Update{}
	mi := &file_acme_app_sample_v1_sample_v1_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Update) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Update) ProtoMessage() {}

func (x *Update) ProtoReflect() protoreflect.Message {
	mi := &file_acme_app_sample_v1_sample_v1_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Update.ProtoReflect.Descriptor instead.
func (*Update) Descriptor() ([]byte, []int) {
	return file_acme_app_sample_v1_sample_v1_proto_rawDescGZIP(), []int{2}
}

func (x *Update) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *Update) GetActor() string {
	if x != nil {
		return x.Actor
	}
	return ""
}

func (x *Update) GetBody() string {
	if x != nil {
		return x.Body
	}
	return ""
}

// Created is the event emitted when we create a sample.
type Created struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Version       int64                  `protobuf:"varint,2,opt,name=version,proto3" json:"version,omitempty"`
	At            int64                  `protobuf:"varint,3,opt,name=at,proto3" json:"at,omitempty"`
	Actor         string                 `protobuf:"bytes,4,opt,name=actor,proto3" json:"actor,omitempty"`
	Body          string                 `protobuf:"bytes,5,opt,name=body,proto3" json:"body,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Created) Reset() {
	*x = Created{}
	mi := &file_acme_app_sample_v1_sample_v1_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Created) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Created) ProtoMessage() {}

func (x *Created) ProtoReflect() protoreflect.Message {
	mi := &file_acme_app_sample_v1_sample_v1_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Created.ProtoReflect.Descriptor instead.
func (*Created) Descriptor() ([]byte, []int) {
	return file_acme_app_sample_v1_sample_v1_proto_rawDescGZIP(), []int{3}
}

func (x *Created) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *Created) GetVersion() int64 {
	if x != nil {
		return x.Version
	}
	return 0
}

func (x *Created) GetAt() int64 {
	if x != nil {
		return x.At
	}
	return 0
}

func (x *Created) GetActor() string {
	if x != nil {
		return x.Actor
	}
	return ""
}

func (x *Created) GetBody() string {
	if x != nil {
		return x.Body
	}
	return ""
}

// Updated is the event emitted when we update a sample.
type Updated struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Version       int64                  `protobuf:"varint,2,opt,name=version,proto3" json:"version,omitempty"`
	At            int64                  `protobuf:"varint,3,opt,name=at,proto3" json:"at,omitempty"`
	Actor         string                 `protobuf:"bytes,4,opt,name=actor,proto3" json:"actor,omitempty"`
	Body          string                 `protobuf:"bytes,5,opt,name=body,proto3" json:"body,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Updated) Reset() {
	*x = Updated{}
	mi := &file_acme_app_sample_v1_sample_v1_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Updated) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Updated) ProtoMessage() {}

func (x *Updated) ProtoReflect() protoreflect.Message {
	mi := &file_acme_app_sample_v1_sample_v1_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Updated.ProtoReflect.Descriptor instead.
func (*Updated) Descriptor() ([]byte, []int) {
	return file_acme_app_sample_v1_sample_v1_proto_rawDescGZIP(), []int{4}
}

func (x *Updated) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *Updated) GetVersion() int64 {
	if x != nil {
		return x.Version
	}
	return 0
}

func (x *Updated) GetAt() int64 {
	if x != nil {
		return x.At
	}
	return 0
}

func (x *Updated) GetActor() string {
	if x != nil {
		return x.Actor
	}
	return ""
}

func (x *Updated) GetBody() string {
	if x != nil {
		return x.Body
	}
	return ""
}

// PUT THE SNAPSHOT (IF USING ONE) in this section
// What is a snapshot?
// A snapshot is a recorded version of the aggregate at a point in time.
// Its main purpose is cost control, but it also helps with archival
// and record retention.
// Snapshots are NOT required, but they are EXTREMELY helpful for the operational
// folks managing the data.
//
// Snapshots facilitate rules such as:
//  1. After every snapshot, delete all older versions.
//  2. After every snapshot, set a TTL on older versions of 30 days so that they delete themselves
//  2. After every snapshot
//  1. Extract, serialize, and write the older records to durable storage
//  2. Delete the older records.
//
// The format of the snapshot must be EXACTLY as detailed below.
// The only change is the name of the aggregate.
type Snapshot struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	Id            string                 `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	Version       int64                  `protobuf:"varint,2,opt,name=version,proto3" json:"version,omitempty"`
	At            int64                  `protobuf:"varint,3,opt,name=at,proto3" json:"at,omitempty"`
	Actor         string                 `protobuf:"bytes,4,opt,name=actor,proto3" json:"actor,omitempty"`
	Snapshot      *Sample                `protobuf:"bytes,5,opt,name=snapshot,proto3" json:"snapshot,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Snapshot) Reset() {
	*x = Snapshot{}
	mi := &file_acme_app_sample_v1_sample_v1_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Snapshot) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Snapshot) ProtoMessage() {}

func (x *Snapshot) ProtoReflect() protoreflect.Message {
	mi := &file_acme_app_sample_v1_sample_v1_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Snapshot.ProtoReflect.Descriptor instead.
func (*Snapshot) Descriptor() ([]byte, []int) {
	return file_acme_app_sample_v1_sample_v1_proto_rawDescGZIP(), []int{5}
}

func (x *Snapshot) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *Snapshot) GetVersion() int64 {
	if x != nil {
		return x.Version
	}
	return 0
}

func (x *Snapshot) GetAt() int64 {
	if x != nil {
		return x.At
	}
	return 0
}

func (x *Snapshot) GetActor() string {
	if x != nil {
		return x.Actor
	}
	return ""
}

func (x *Snapshot) GetSnapshot() *Sample {
	if x != nil {
		return x.Snapshot
	}
	return nil
}

var File_acme_app_sample_v1_sample_v1_proto protoreflect.FileDescriptor

const file_acme_app_sample_v1_sample_v1_proto_rawDesc = "" +
	"\n" +
	"\"acme/app/sample/v1/sample_v1.proto\x12\x12acme.app.sample.v1\x1a5funinthecloud/protosource/options/v1/options_v1.proto\"\xc2\x01\n" +
	"\x06Sample\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x18\n" +
	"\aversion\x18\x02 \x01(\x03R\aversion\x12\x1b\n" +
	"\tcreate_at\x18\x03 \x01(\x03R\bcreateAt\x12\x1b\n" +
	"\tcreate_by\x18\x04 \x01(\tR\bcreateBy\x12\x1b\n" +
	"\tmodify_at\x18\x05 \x01(\x03R\bmodifyAt\x12\x1b\n" +
	"\tmodify_by\x18\x06 \x01(\tR\bmodifyBy\x12\x12\n" +
	"\x04body\x18\a \x01(\tR\x04body:\x06ʬ\x1d\x02\x1a\x00\"U\n" +
	"\x06Create\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x14\n" +
	"\x05actor\x18\x02 \x01(\tR\x05actor\x12\x12\n" +
	"\x04body\x18\x03 \x01(\tR\x04body:\x11ʬ\x1d\r\n" +
	"\v\n" +
	"\aCreated\x10\x01\"U\n" +
	"\x06Update\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x14\n" +
	"\x05actor\x18\x02 \x01(\tR\x05actor\x12\x12\n" +
	"\x04body\x18\x03 \x01(\tR\x04body:\x11ʬ\x1d\r\n" +
	"\v\n" +
	"\aUpdated\x10\x02\"u\n" +
	"\aCreated\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x18\n" +
	"\aversion\x18\x02 \x01(\x03R\aversion\x12\x0e\n" +
	"\x02at\x18\x03 \x01(\x03R\x02at\x12\x14\n" +
	"\x05actor\x18\x04 \x01(\tR\x05actor\x12\x12\n" +
	"\x04body\x18\x05 \x01(\tR\x04body:\x06ʬ\x1d\x02\x12\x00\"u\n" +
	"\aUpdated\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x18\n" +
	"\aversion\x18\x02 \x01(\x03R\aversion\x12\x0e\n" +
	"\x02at\x18\x03 \x01(\x03R\x02at\x12\x14\n" +
	"\x05actor\x18\x04 \x01(\tR\x05actor\x12\x12\n" +
	"\x04body\x18\x05 \x01(\tR\x04body:\x06ʬ\x1d\x02\x12\x00\"\x9c\x01\n" +
	"\bSnapshot\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x18\n" +
	"\aversion\x18\x02 \x01(\x03R\aversion\x12\x0e\n" +
	"\x02at\x18\x03 \x01(\x03R\x02at\x12\x14\n" +
	"\x05actor\x18\x04 \x01(\tR\x05actor\x126\n" +
	"\bsnapshot\x18\x05 \x01(\v2\x1a.acme.app.sample.v1.SampleR\bsnapshot:\bʬ\x1d\x04*\x02\b2B\xda\x01Ҭ\x1d\x02\b\x01\n" +
	"\x16com.acme.app.sample.v1B\rSampleV1ProtoP\x01Z@github.com/funinthecloud/protosource/acme/app/sample/v1;samplev1\xa2\x02\x03AAS\xaa\x02\x12Acme.App.Sample.V1\xca\x02\x12Acme\\App\\Sample\\V1\xe2\x02\x1eAcme\\App\\Sample\\V1\\GPBMetadata\xea\x02\x15Acme::App::Sample::V1b\x06proto3"

var (
	file_acme_app_sample_v1_sample_v1_proto_rawDescOnce sync.Once
	file_acme_app_sample_v1_sample_v1_proto_rawDescData []byte
)

func file_acme_app_sample_v1_sample_v1_proto_rawDescGZIP() []byte {
	file_acme_app_sample_v1_sample_v1_proto_rawDescOnce.Do(func() {
		file_acme_app_sample_v1_sample_v1_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_acme_app_sample_v1_sample_v1_proto_rawDesc), len(file_acme_app_sample_v1_sample_v1_proto_rawDesc)))
	})
	return file_acme_app_sample_v1_sample_v1_proto_rawDescData
}

var file_acme_app_sample_v1_sample_v1_proto_msgTypes = make([]protoimpl.MessageInfo, 6)
var file_acme_app_sample_v1_sample_v1_proto_goTypes = []any{
	(*Sample)(nil),   // 0: acme.app.sample.v1.Sample
	(*Create)(nil),   // 1: acme.app.sample.v1.Create
	(*Update)(nil),   // 2: acme.app.sample.v1.Update
	(*Created)(nil),  // 3: acme.app.sample.v1.Created
	(*Updated)(nil),  // 4: acme.app.sample.v1.Updated
	(*Snapshot)(nil), // 5: acme.app.sample.v1.Snapshot
}
var file_acme_app_sample_v1_sample_v1_proto_depIdxs = []int32{
	0, // 0: acme.app.sample.v1.Snapshot.snapshot:type_name -> acme.app.sample.v1.Sample
	1, // [1:1] is the sub-list for method output_type
	1, // [1:1] is the sub-list for method input_type
	1, // [1:1] is the sub-list for extension type_name
	1, // [1:1] is the sub-list for extension extendee
	0, // [0:1] is the sub-list for field type_name
}

func init() { file_acme_app_sample_v1_sample_v1_proto_init() }
func file_acme_app_sample_v1_sample_v1_proto_init() {
	if File_acme_app_sample_v1_sample_v1_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_acme_app_sample_v1_sample_v1_proto_rawDesc), len(file_acme_app_sample_v1_sample_v1_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   6,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_acme_app_sample_v1_sample_v1_proto_goTypes,
		DependencyIndexes: file_acme_app_sample_v1_sample_v1_proto_depIdxs,
		MessageInfos:      file_acme_app_sample_v1_sample_v1_proto_msgTypes,
	}.Build()
	File_acme_app_sample_v1_sample_v1_proto = out.File
	file_acme_app_sample_v1_sample_v1_proto_goTypes = nil
	file_acme_app_sample_v1_sample_v1_proto_depIdxs = nil
}
