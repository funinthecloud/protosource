...
// "example.app.sample.v1.{Operation}" where {Operation} is:
...
// HandleLoad retrieves the current state of a Sample aggregate by
...
func (h *Handler) HandleLoad(ctx context.Context, request protosource.Request) protosource.Response {
	ctx, err := h.authorizer.Authorize(ctx, request, "example.app.sample.v1.LoadSample")
	if err != nil {
		return authzErrorResponse(request, err)
	}
...
// HandleGet retrieves the Sample aggregate from the materialized store.
...
func (h *Handler) HandleGet(ctx context.Context, request protosource.Request) protosource.Response {
	ctx, err := h.authorizer.Authorize(ctx, request, "example.app.sample.v1.GetSample")
	if err != nil {
		return authzErrorResponse(request, err)
	}
...
func (h *Handler) HandleHistory(ctx context.Context, request protosource.Request) protosource.Response {
	ctx, err := h.authorizer.Authorize(ctx, request, "example.app.sample.v1.HistorySample")
	if err != nil {
		return authzErrorResponse(request, err)
	}
...
func (h *Handler) HandleQueryByCreateBy(ctx context.Context, request protosource.Request) protosource.Response {
	ctx, err := h.authorizer.Authorize(ctx, request, "example.app.sample.v1.QueryByCreateBy")
	if err != nil {
		return authzErrorResponse(request, err)
	}
	...
	if create_byRaw == "" {
		return errorResponse(request, http.StatusBadRequest, "QUERY_MISSING_PK", "missing required parameter: create_by", nil)
	}
